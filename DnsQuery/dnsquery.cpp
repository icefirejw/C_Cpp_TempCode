...
QUERYERROR:
    ...
    if (tmpbuf != NULL) delete [] tmpbuf;
    if (DNS_ERR_WINSOCK != errerno)  WSACleanup();
...
void DnsQuery::PrintDnsResult(void)
{
    ...
    if (DnsQueryData.DnsHeader.ans_count > 0) {
        cout<<endl;
    ...
    if (DnsQueryData.DnsHeader.auth_count > 0) {
        cout<<endl;
    ...
    if (DnsQueryData.DnsHeader.add_count) {
        cout<<endl;
